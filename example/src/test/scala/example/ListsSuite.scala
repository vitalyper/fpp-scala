...
class ListsSuite extends FunSuite {
  ...
  test("sum of unit list") {
    assert(sum(List(expectedInt)) === expectedInt)
  }

  test("maxOfTwo both equal") {
    assert(maxOfTwo(expectedInt, expectedInt) === expectedInt)
  }

  test("maxOfTwo x is less") {
    assert(maxOfTwo(0, expectedInt) === expectedInt)
  }
  ...
  test("max of unit list") {
    assert(max(List(expectedInt)) === expectedInt)
  }
}
