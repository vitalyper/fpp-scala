...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = 
    (for (p <- b.legalNeighbors) yield ((p._1, p._2 :: history))).toStream

  /**
   * This function returns the list of neighbors without the block
   * positions that have already been explored. We will use it toc
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more = for {
        p <- initial
        ns <- newNeighborsOnly(neighborsWithHistory(p._1, p._2), explored)
      } yield ns
  ...
  lazy val solution: List[Move] = {
    val solutions = pathsToGoal.map(p => p._2).sortBy(_.length)
    if (solutions.isEmpty) List() else solutions.head
  }
}
